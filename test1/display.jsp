...
<%
    ...
    java.util.List<Object> userList = (java.util.List<Object>) session.getAttribute("userList");
...
<head>
    ...
    <style>
        ...
        .search-container { flex-grow: 1; margin: 0 20px; max-width: 300px; display: flex; align-items: center; position: relative; }
        ...
        .add-btn { background: #28a745; color: white; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; border-radius: 50%; box-shadow: 0 4px 10px rgba(40, 167, 69, 0.3); }
        ...
        const userData = [
            <% if (userList != null && !userList.isEmpty()) {
                for (int i = 0; i < userList.size(); i++) {
                    ...
                    out.print("[\"" + 
                        (getName.invoke(user) != null ? ((String) getName.invoke(user)).replace("\"", "\\\"") : "N/A") + "\",\"" +
                        (getEmail.invoke(user) != null ? ((String) getEmail.invoke(user)).replace("\"", "\\\"") : "N/A") + "\",\"" +
                        (getUsername.invoke(user) != null ? ((String) getUsername.invoke(user)).replace("\"", "\\\"") : "N/A") + "\"]");
        ...
        <% if (userList != null && !userList.isEmpty()) { %>
            <div class="table-title">
                ...
                <div class="search-container">
                    ...
                    <button class="add-btn" onclick="location.replace('/test1');" title="Add New User">
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        for (Object user : userList) {
                            String rowId = String.valueOf(getId.invoke(user)); %>
                        <tr id="row-<%= rowId %>">
                            <td><%= serialNumber++ %></td>
                            <td><%= getName.invoke(user) != null ? getName.invoke(user) : "N/A" %></td>
                            <td><%= getEmail.invoke(user) != null ? getEmail.invoke(user) : "N/A" %></td>
                            <td><%= getUsername.invoke(user) != null ? getUsername.invoke(user) : "N/A" %></td>
                            <td>
